...
const HomePageBottomHeader = () => {
	...
	return (
    <nav
      className={`hidden slg:flex justify-center pr-10 gap-24 items-center w-full py-1 bg-white transition`}
    >
      <div className="flex w-fit gap-8 overflow-hidden">
        {headerNavLinks.map((link) => (
          <Link
            key={link.id}
            href={link.href}
            className={`text-base min-w-fit font-medium leading-[1.8] transition hover:text-effect relative group ${
